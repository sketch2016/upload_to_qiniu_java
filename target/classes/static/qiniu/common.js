...
function imageDeal(board, key, domain) {
    ...
    var baseUrl = "http://localhost:9999/downloadtoken?baseUrl=" + url;
    $.ajax({url:baseUrl,async:true,success:function(res){
            console.log('res:'+ res)
            $(board)
                .find(".wraper a")
                .html(
...
function storeUrl(key, url) {
    var encodeUrl = url.replace('&', '%26');
    console.log('storeUrl:encodeUrl = ' + encodeUrl)

    $.ajax({
        url:'http://localhost:9999/storeDownloadUrl?key=' + key + '&downloadUrl=' + encodeUrl,
        async:true,
        success: function (res) {
            console.log('storeUrl success, res = ' + res)
        }
    });
}

function zipDeal(board, key, domain) {
    var baseUrl = 'http://' + domain + '/' + key;
    $.ajax({url:"http://localhost:9999/downloadtoken?baseUrl=" + baseUrl,async:true,success:function(res){
            console.log('res:'+ res)
            $(board)
                .find(".wraper a")
                .html(
